...
def get_closest(inp, string, indexes, tabs, threshold, vocab, stop_words):
    ...
    feature = [len_string]
    # Proportion
    feature.append(-dist / (len_string + dist + 0.0) * 4)
    ...
    for w, p in zip(inp, pos_tags):
        if (w in backbone) and (
            ...
            if buf == "":
                ...
                if not proposed:
                    closest = get_closest(
                        inp, buf, last, tabs, threshold, vocab, stop_words
                    )
                    if closest:
                        buf = "@#{};{},{}@#".format(
                            recover(
            ...
            if buf == "":
                ...
                if not proposed:
                    closest = get_closest(inp, buf, last, tabs, threshold, vocab, stop_words)
                    if closest:
                        buf = "@#{};{},{}@#".format(
                            recover(
                                buf,
...
def get_lemmatize(words, return_pos):
    ...
    recover_dict = {}
    words = [x for x in words.strip().split(" ") if x]
...
def merge_strings(string, tags=None):
    ...
    for idx in range(len(string)):
        c = string[idx]
        last_cc = string[idx - 1:idx + 1] if idx > 0 else ""
        if last_cc == "@#" and not inside:
            inside = True
            buff += c
        elif last_cc == "@#" and inside:
            inside = False
            buff += c
            words.append(buff)
            buff = ""
    ...
    while i < len(words):
        if i < 2:
            ...
            if is_number(words[i].split(";")[0][1:]) and is_number(
                words[i - 2].split(";")[0][1:]
            ):
                i += 1
            else:
                prev_idx = words[i - 2].rsplit(";", 1)[1][:-2].split(",")
                cur_idx = words[i].rsplit(";", 1)[1][:-2].split(",")
                
                if cur_idx == prev_idx or (
                    prev_idx[0] == "-2" and prev_idx[1] == cur_idx[1]
                ):
...
def sub_func(inputs, vocab, stop_words):
    df, entry, name = inputs
    backbone = {}
    trans_backbone = {}
    transliterate = {}
    tabs = []
    ...
    # Masking the caption
    captions, row = get_lemmatize(entry[2].strip(), False)
    ...
    for i in range(len(entry[0])):
        ...
        if not results:
            results = [[sent], [entry[1][i]], [tags], entry[2], entry[3]]
        else:
...
def get_func(filename, vocab, stop_words):
    dfs = []
    entries = []
    names = []

    func = functools.partial(sub_func, vocab=vocab, stop_words=stop_words)

    with jsonlines.open(filename, "r") as reader:
        for idx, jobj in enumerate(reader):
            df = pd.DataFrame(json.loads(jobj["table"]))
            if df.empty:
                continue
            dfs.append(df)
            entries.append(([jobj["positive"], jobj["negative"]], [1, 0], jobj["title"], jobj["highlighted_cells"]))
            names.extend([f"{idx}"])
    ...
    # for input_ in zip(dfs, entries, names):
    #     func(input_)


    r = pool.map(func, zip(dfs, entries, names))
...
def main(cfg):
    print("Start to process {}".format(cfg.input_file))

    with open(cfg.vocab_file) as f:
        vocab = json.load(f)

    with open(cfg.stop_words_file) as f:
        stop_words = json.load(f)
    data = get_func(cfg.input_file, vocab, stop_words)
