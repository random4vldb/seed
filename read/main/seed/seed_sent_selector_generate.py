...
def _retrieve_one(
    ...
    inst_id, query, dpr_query, positive_pids, positive_context, answers, table, _, _ = query_tuple
    normed_positives = [
        " " + p["title"] + "  " + p["text"] + " "
    ...
    def _write_batch(self, out, query_tuples, all_negs):
        for query_tuple, negs in zip(query_tuples, all_negs):
            ...
            for pndx, pos in enumerate(positive_context[:num_instances]):
                if answer in pos["text"]:
                    # print("Good answer", answer, pos['text'])
                    for sent in text_to_sentences(pos["text"]).split("\n"):
                        sent = sent.strip().replace("BULLET: -", "")
                        if sent == answer.strip():
                            continue
            ...
            negative_sentences = []
            for neg in negs[pndx::num_instances][: self.cfg.max_negatives]:
                for sent in text_to_sentences(neg["text"]).split("\n"):
                    sent = sent.strip().replace("BULLET: -", "")
                    negative_sentences.append(sent)
            if negative_sentences:
                out.write(
                ...
                for jobj in reader:
                    ...
                    query_tuples.append(
                        (
                            ...
                            answers,
                            table,
                            jobj["table_page_title"],
                            jobj["highlighted_cells"]
                        )
                    )
...
@hydra.main(
    version_base="1.2",
    config_path=root / "config" / "seed",
    config_name="seed_sent_generate.yaml",
)
def main(cfg):
    random.seed(21)
    bm25dpr = BM25forDPR(cfg.pyserini_index, cfg)
    for file in Path(cfg.positive_pids).glob("*.jsonl"):
        bm25dpr.create(file, Path(cfg.output_dir) / file.name)
