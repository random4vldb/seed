...
@Step.register("totto::filter")
class TottoFilter(Step):
    DETERMINISTIC: bool = True
    CACHEABLE: Optional[bool] = True
    FORMAT: Format = JsonFormat()

    def run(self, data_dir, filtered_dir, augmented_dir, triplet_dir):
        ...
        for data_file in data_dir.iterdir():
            ...
            saved_df.to_json(Path(filtered_dir) / data_file.name, orient="records", lines=True)

            final_df = generate(df)

            logger.info("Final df with len(df) = {} and {} negatives".format(len(final_df), len(final_df) - len(df)))

            Path(augmented_dir).mkdir(exist_ok=True, parents=True)
            ...
            final_df.to_json(Path(augmented_dir) / data_file.name, lines=True, orient="records")
            ...
            Path(triplet_dir).mkdir(exist_ok=True, parents=True)

            with jsonlines.open(Path(triplet_dir) / data_file.name, "w") as writer:
